// Task 3.6. Lifted verbatim from lib/templates/atlas/styles.ts (deleted whole in 3.8, per build
// plan §7) — this file had zero dependency on anything inside lib/templates/ itself (only
// lib/content/normalize-brand-colors.ts and lib/design/resolve-tokens.ts, both untouched by
// 3.8), so the CSS content below is unchanged from atlas's own, battle-tested stylesheet, not
// rewritten. The one deliberate change: every `.tpl-atlas`/`body.tpl-atlas` selector (6
// occurrences in the original) is gone — that scoping existed so atlas's CSS could coexist
// safely with ledger's and showcase's inside the old shared TemplateGallery/registry.ts, a
// concern that no longer applies once this is the one and only renderer producing this page.
// Same simplification 3.3's generate-stylesheet.ts already made for the same reason ("one
// concept = one stylesheet, no multi-template coexistence concern"). The `.at-*` component
// classes themselves are untouched — renaming ~500 lines of a working, previously-shipped
// stylesheet for cosmetic reasons risked introducing a real bug for no real benefit.

import type { Palette } from "../content/normalize-brand-colors";
import { type TemplateTokens, scaledBand } from "../design/resolve-tokens";

export function fallbackStyles(p: Palette, tokens: TemplateTokens): string {
  return `
:root {
  --accent: ${p.accent};
...
a:focus-visible, summary:focus-visible { outline: var(--focus-ring-width) solid var(--ring); outline-offset: 3px; }
