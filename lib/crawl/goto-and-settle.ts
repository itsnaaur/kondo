...
// Task 3.8. Corrected — the previous version of this comment named lib/crawl/visual-
// shots.ts and lib/crawl/reference-screenshot.ts, neither of which exists in this repo
// (both belonged to the pre-rebuild "AI website engine," removed whole in an earlier
// migration). The real, current caller of gotoAndSettle is lib/crawl/crawler.ts.
//
...
// inheriting networkidle's unbounded hang on sites that never go quiet. Sharing this one
// helper is what keeps every real caller on the same strategy rather than each hand-
